...
# 🔴 IMPORTANT: import from modelsimple, not model
from modelsimple import build_model
...
def train():
    ...
    # 🔹 Optional but recommended: use a separate folder for the simple model
    cfg.save_dir = os.path.join(cfg.save_dir, "simple_cnn")

    # 🔹 Optional: force training from scratch for this experiment
    ...
    criterions = {
        k: nn.CrossEntropyLoss(label_smoothing=cfg.label_smoothing)
    ...
    if cfg.resume_from:
        ...
        # Auto-detect best checkpoint in the *simple_cnn* folder
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        for batch in pbar:
            ...
            # labels per head
            labels = {
                k: batch[f"label_{k}"].to(device)
                for k in num_classes.keys()
            }
        ...
        if macro_acc > best_macro_acc:
            ...
            ckpt = os.path.join(
                cfg.save_dir, f"best_multitask_simple_macro{best_macro_acc:.4f}.pt"
            )
